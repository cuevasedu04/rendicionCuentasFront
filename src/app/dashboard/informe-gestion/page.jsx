    ...
    >
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500 max-w-[55ch]">
          Personal de nivel 3-5 sin manejo de recursos asignados.
        </p>
      </div>

      {/* Stats — asymmetric 2-col (DESIGN_VARIANCE: 8) */}
      <div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-5">

        {/* En Plazo — primary stat */}
        <motion.div
            ...
            {loading.excedidos
              ...
              : topExcedidas.map(([nombre, empleados], i) => (
                <motion.div
                  ...
                  className="flex items-center gap-3 px-5 py-3 hover:bg-zinc-50/70 transition-colors cursor-pointer group"
                  onClick={() => onNavigate('excedidos')}
                >
                  ...
                  <span className="flex-1 text-sm text-zinc-700 truncate">{nombre}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[272px_1fr] gap-4 items-start">
        ...
        <div className="lg:sticky lg:top-[184px]">
          <div className="rounded-2xl bg-white/80 border border-zinc-200/80 overflow-hidden shadow-[0_1px_4px_rgba(0,0,0,0.04)] backdrop-blur-sm">
            ...
            <div className="p-2 space-y-0.5 max-h-[55vh] overflow-y-auto scrollbar-thin">
              {loading
                ? [...Array(5)].map((_, i) => (
        ...
        <div className="rounded-2xl bg-white/80 border border-zinc-200/80 overflow-hidden shadow-[0_1px_4px_rgba(0,0,0,0.04)] backdrop-blur-sm">
          ...
          {/* Column headers — must match EmpleadoRow grid template exactly */}
...
export default function InformeGestionPage() {
  ...
  const handleRefresh = (tipo) => {
    ...
    const api = createApi(session.drfToken);
    const call = tipo === 'activos' ? api.informeGestion.activos() : api.informeGestion.excedidos();
    call
