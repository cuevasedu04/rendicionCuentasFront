...
function diasColor(restantes) {
  if (restantes >= 8) return { bar: 'bg-emerald-500', badge: 'bg-emerald-100 text-emerald-800' };
  if (restantes >= 4) return { bar: 'bg-amber-400', badge: 'bg-amber-100 text-amber-800' };
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <div className="flex flex-col items-end gap-1 flex-shrink-0">
          ...
          {!esNivel12 && (
            <span className="px-2 py-0.5 rounded-full text-xs bg-[#621f32]/10 text-[#621f32] font-medium">
              maneja recursos
            </span>
          )}
        </div>
    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5">
          <BuildingOffice2Icon className="h-4 w-4 text-[#621f32] flex-shrink-0" />
...
export default function ActaEntregaPage() {
  ...
  const fetchTab = async (tab) => {
    ...
    try {
      const res = await (tab === 'activos' ? api.actaEntrega.activos() : api.actaEntrega.excedidos());
      setData((d) => ({ ...d, [tab]: res }));
    } catch (e) {
        ...
        {activeTab ? (
          ...
          >
            ...
            {!currentLoading && !currentError && currentData && grupoKeys.length === 0 && (
              <div className="flex flex-col items-center justify-center py-20 text-zinc-400">
                <ClipboardDocumentCheckIcon className="h-10 w-10 mb-3 opacity-35" />
          ...
          >
            <div className="flex gap-4 mb-4">
              <ClipboardDocumentCheckIcon className="h-9 w-9 text-zinc-300" />
